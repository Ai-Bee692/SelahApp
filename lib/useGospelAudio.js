...
function scheduleAfrobeats(ctx, notes, t, spb, destinations, percStyle, isFillBar, volScale) {
  const { piano, percussion } = destinations;
...
function scheduleHighlife(ctx, notes, t, spb, destinations, percStyle, isFillBar, volScale) {
  const { piano, percussion, guitar } = destinations;
  if (percStyle === "mute") return;

  const gVol = 0.18 * volScale;
  const hVol = 0.1 * volScale;

  if (percStyle !== "solo" && guitar) {
    guitarStrum(ctx, notes, t, spb, gVol, guitar);
    guitarStrum(ctx, notes, t + spb * 0.75, spb * 0.5, gVol * 0.6, guitar);
    guitarStrum(ctx, notes, t + spb * 1.5, spb, gVol, guitar);
    guitarStrum(ctx, notes, t + spb * 2, spb, gVol * 0.8, guitar);
    guitarStrum(ctx, notes, t + spb * 3, spb, gVol, guitar);
  }
...
function scheduleContemporary(ctx, notes, t, spb, barDur, destinations, percStyle, isFillBar, volScale) {
  ...
  if (isFillBar && percStyle !== "light") {
    ...
    for (let b = 0; b < 4; b++) {
      ...
      g.gain.setValueAtTime(b === 0 ? 0.2 * volScale : 0.1 * volScale, t + b * spb);
      g.gain.exponentialRampToValueAtTime(0.001, t + b * spb + 0.04);
      osc.connect(g); 
      g.connect(percussion);
      osc.start(t + b * spb); 
      osc.stop(t + b * spb + 0.05);
    }
  }
}
